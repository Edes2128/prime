...
export default function ShtoPorosi({ closePop, refreshData }) {
    ...
    let cmimet;
    if (dokumente !== '') {
        cmimet = dokumente.filter(dok => dok.file.split('.')[1] !== 'pdf' && dok.file.split('.')[1] !== 'docx');
    }
    let kataloget;
    if (dokumente !== '') {
        kataloget = dokumente.filter(dok => dok.file.split('.')[1] === 'pdf');
    }
                ...
                <div className="shtoporosi-pop-content-buttons flex ai-center">
                    ...
                    {file ?
                        <div className="flex ai-center" >
                            <input className="input-porosi-text" type="text" style={{ width: fileName.length + 'ch', }} value={fileName} onChange={(e) => setFilename(e.target.value)} />
                            <button className="porosi-btn fs-18 fw-regular" onClick={() => makeOrder()} >Dergo</button>
                            <button className="porosi-btn anullo-btn-porosi fs-18 fw-regular" onClick={() => {
                                setFile('')
                                setFilename('')
                            }} >Anullo</button>
                        </div>
                        :
                        <>
                            <label className="shtoporosi-pop-content-buttons-porosit fs-18 fw-regular" htmlFor="porosi"  >Bej Porosine</label>
                            <input type="file" hidden id="porosi" onChange={(e) => {
